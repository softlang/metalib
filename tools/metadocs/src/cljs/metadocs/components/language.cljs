...
(defn component [language remove]
  (let [edit? (reagent/atom false)]
    (fn []
      (if @edit?
        [:div {:class "field has-addons"}
         [:div {:class "control"}
          [:input {:class "input" :on-change #(reset! language (-> % .-target .-value)) :value @language}]]
         [:div {:class "control"}
          [:button {:class "button" :on-click #(reset! edit? false)}
           [:i {:class "far fa-check-circle"}]]]]
        [:div {:class "perspective tags has-addons"}
         [:a {:class "tag is-link" :href (str config/base-path "/languages/" (url/sanitize @language) ".html")} @language]
